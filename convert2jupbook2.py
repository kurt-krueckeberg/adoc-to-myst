...
def render_myst_ref(label, target):
    """Render an internal cross-reference for Jupyter Book 2 / MyST Document Engine.

    MyST Document Engine uses Markdown link syntax for local targets, e.g.
    [image 126](#image126).  The Sphinx role form {ref}`label <target>` is
    not portable to JB2/MyST Document Engine and is parsed as one unresolved
    target string.
    """
...
def inline_anchor_targets(elem):
    ...
    This handles malformed or non-bibliography DocBook, such as an AsciiDoc
    [biliography] typo that makes Asciidoctor emit a normal list item whose
    paragraph contains an inline <anchor>.  Target syntax is block-level MyST,
    so it must be emitted before the paragraph/list item, never inside it.
    """
    targets = []
    for child in elem.iter("anchor"):
...
def convert_complex_table(elem, current_doc, caption):
    ...
    if html_path is None or OUTPUT_MD_PATH is None:
        # Defensive fallback for direct/library use without an output path.
        # Normal CLI conversion always has OUTPUT_MD_PATH, so complex tables are
        # written to _table-artifacts and included from the Markdown page.
        out += "```{raw} html\n"
        out += html_text.rstrip() + "\n"
        out += "```\n\n"
        return out

    rel_path = os.path.relpath(str(html_path), start=str(OUTPUT_MD_PATH.parent)).replace("\\", "/")
    out += f"```{{include}} {rel_path}\n"
    out += "```\n\n"
    return out
...
def render_title_with_subtitle(elem, current_doc):
    """Render a DocBook title together with its DocBook subtitle.

    Asciidoctor can split an AsciiDoc document title of the form
    "Title: Subtitle" into sibling <title> and <subtitle> elements in
    DocBook. Treating <title> as the whole heading silently loses the
    subtitle, so all heading-like conversions should go through this helper.
    """
    if elem is None:
        return ""
    ...
    title = render_inline(title_elem, current_doc).strip()

    subtitle_elem = elem.find("subtitle")
    subtitle = render_inline(subtitle_elem, current_doc).strip() if subtitle_elem is not None else ""

    if title and subtitle:
        return f"{title} {subtitle}" if title.endswith(":") else f"{title}: {subtitle}"
...
def render_inline(elem, current_doc):
    ...
    for child in elem:
        if child.tag == "emphasis":
            ...
            # Anchors define block-level targets.  Do not emit `(id)=` inside
            # inline text.  If Asciidoctor represented `[[[id]]]` as text
            # `[` + <anchor id="id"/> + `]`, preserve the visible citation
            # label as `[id]` while the target itself is emitted separately.
            anchor_id = elem_id(child)
            tail = child.tail or ""
            if anchor_id and out.endswith("[") and tail.startswith("]"):
                out = out[:-1] + f"[{anchor_id}]"
                tail = tail[1:]
            if tail:
                out += tail
...
def convert_sidebar(elem, current_doc):
    title = render_title_with_subtitle(elem, current_doc) or "Sidebar"
...
def convert_bibliography(elem, current_doc, level=1):
    ...
    if title is not None:
        out += "#" * level + " " + render_title_with_subtitle(elem, current_doc) + "\n\n"
    for child in elem:
...
def convert_element(elem, current_doc, level=1):
    ...
    if elem.tag in ("section", "sect1", "sect2"):
        title = elem.find("title")
        if title is not None:
            title_text = render_title_with_subtitle(elem, current_doc)
            out += "#" * level + " " + title_text + "\n\n"
        for child in elem:
            if child.tag in ("title", "subtitle"):
                continue
    ...
    if elem.tag == "formalpara":
        title = elem.find("title")
        if title is not None:
            title_text = render_title_with_subtitle(elem, current_doc).strip()
            if title_text:
                out += title_text + "\n\n"
        for child in elem:
            if child.tag in ("title", "subtitle"):
    ...
    if elem.tag == "bibliomisc":
        targets = "".join(target + "\n\n" for target in inline_anchor_targets(elem))
        text = render_inline(elem, current_doc).strip()
        return out + targets + (text + "\n\n" if text else "")
    if elem.tag == "anchor":
...
def convert(doc, current_doc, output_md_path):
    ...
    if title_parent is not None:
        title_text = render_title_with_subtitle(title_parent, current_doc)
        if title_text:
            out += "# " + title_text + "\n\n"

    for child in root:
        if child.tag in ("title", "subtitle", "info"):
            continue
